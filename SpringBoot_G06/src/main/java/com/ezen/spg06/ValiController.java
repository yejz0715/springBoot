...
public class ValiController {
	...
	public String create(@ModelAttribute("dto") ContentDto contentdto, Model model,
		...
		//BindingResult:키값(제목)과 벨류값(내용)으로 구성된 오류 내용 저장 클래스
		//validate 메서드는 contentdto와 BindingResult 형태의 레퍼런스변수를 매개변수로 넣고 시작
		//validator의 멤버메서드인 validate 가 contentdto 내용을 검사한 후  result에 오류 내용을 담아주고,
		//리턴되지않아도 call by reference 이기때문에 오류내용을 현재 위치에서도 result라는 이름으로 사용가능
